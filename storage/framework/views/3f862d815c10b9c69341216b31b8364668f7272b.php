                ...
                <div class="card bg-secondary shadow">
                    <div class="card-body">
                        ...
                        <div class="pl-lg-4">
                           ...
                           <!--<h1><?php echo e(auth()->user()->restorant->user->api_token); ?></h1>-->
                            <!--<div class="row mt-3">-->
                            <!--    <div class="col-md-6">-->
                            <!--        <label class="form-control-label"  for="ress">Choose a Restaurant:</label>-->
                            <!--        <select name="ress" class=" form-control" id="ress">-->
                            <!--               <option value="">select</option>-->
                            <!--        </select>-->
                            <!--         <button type="button" id="loading-res" class="btn btn-sm btn-primary">Loading...</button>-->
                            <!--    </div>-->
                            <!--</div>-->
                            <!-- <div class="row mt-3">-->
                            <!--    <div class="col-md-6">-->
                            <!--    <label class="form-control-label" for="name">API Key</label>-->
                            <!--    <input  type="text" name="name" id="api" class="form-control form-control   " placeholder="API Key"  required="" autofocus="">-->
                            ...
                            <div id="discount-div">
                             ...
                             <div class="row mt-3">
                                <div class="col-md-6">
                                    <label class="form-control-label"  for="diss">Choose a Discount Category:</label>
                            ...
                            <div class="form-group  mt-3">
                               <div class="custom-control custom-checkbox">
                                ...
                                <label class="custom-control-label" for="dinein">Dine in</label>
                                </div>
                             </div>
                             ...
                             <div class="form-group  ">
                                <label class="form-control-label" for="excerpt">Validity note</label>
                                <textarea maxlength="38"  placeholder="Character Limit 38" class="form-control" name="excerpt" id="excerpt"></textarea>
                             </div>
                             <div class="row mt-3">
                                <div class="col-md-6">
                                    <label class="form-control-label"   for="opts">Apply to Selected Item:</label>
                                    <input type="radio"  id="opts" checked name="opts" value="0" />
                                </div>
                                 <div class="col-md-6">
                                    <label class="form-control-label"  for="opts">Apply to All Items:</label>
                            ...
                            <div class="row mt-3" id="cats-div" style="display:none;">
                                <div class="col-md-6">
                                    <label class="form-control-label"  for="cats">Choose Food category:</label>
                             ...
                             <div class="row mt-3" id="items-div">
                                <div class="col-md-6">
                                    <label class="form-control-label"  for="items">Choose an item:</label>
                                    <select name="items[]" class="js-example-basic-multiple form-control" multiple="multiple" id="items">
                                           <option value="">select</option>
                                    </select>
                                        <button type="button" id="loading-res-1" class="btn btn-sm btn-primary">Loading...</button>
                              ...
                              <!--   <div class="row mt-3">
                                ...
                                <input step=".01" type="text" name="name" id="name" class="form-control form-control   " placeholder="Restaurant Name" value="Restaurant 3" required="" autofocus="">
    
                                </div>cjDtmn6nP1mV7o7r
                                
                            </div>-->
                            <div class="row mt-3 ml-1">
                               <button type="button" id="submit-btn" class="btn btn-sm btn-success"><?php echo e(__('Sync Data')); ?></button>
                             <button type="button" id="loading" class="btn btn-sm btn-primary">Loading...</button>
                           
                            </div>
                            </div>
                             <button type="button"  class="btn btn-lg btn-success" id="checking">Loading, please be patient....</button>
...
<script>
    ...
    // $.getJSON("https://yummy-hunt.com/api/v2/client/restaurants", function(result){
    //     console.log(result)
    //     $("#loading-res").hide();
    //       $("#ress").show();
    ...
    // $("#excerpt").change(e => {
    //     if($("#excerpt").val().toString().length > 38){
    //         $("#excerpt").val($("#excerpt").val().toString().slice(0,38))
    //     }
    // })
    
    $.getJSON("https://dealont.com/index.php/wp-json/listar/v1/category/list", function(result){
        console.log(result)
         $("#loading-res").hide();
          $("#diss").show();
    ...
    $.getJSON("https://dealont.com/index.php/wp-json/listar/v1/place/list?s=<?php echo e(auth()->user()->restorant['name']); ?>", function(result){
        console.log(result?.data)
        if(result?.data?.length > 0){
            let res = result?.data[0];
            res_id = res?.ID;
            if(res?.post_title === "<?php echo e(auth()->user()->restorant['name']); ?>"){
                $.getJSON("https://dealont.com/index.php/wp-json/listar/v1/place/view?id="+res?.ID, function(result2){
    ...
    //http://dealont.com/index.php/wp-json/listar/v1/place/list?s=Restaurant 3
        ...
        $("#submit-btn").hide();
         $("#loading").show();
        $.getJSON("https://yummy-hunt.com/api/v2/client/items/<?php echo e(auth()->user()->restorant['id']); ?>", function(result){
       console.log(result.data)
       $("#submit-btn").show();
         $("#loading").hide();
         $("#items").empty();
   ...
   $('input[type=radio]').click(function(){
    ...
    function perItem(item_id,diss) {
        ...
        const api_key ='<?php echo e(auth()->user()->api_token); ?>';
         ...
         $("#submit-btn").hide();
         $("#loading").show();
        $.getJSON(`https://yummy-hunt.com/api/v2/client/discount/${item_id}/${diss}?api_key=${api_key}`, function(result){
      console.log(result)
       listarSave();
    });
    }
    ...
    function perSelectedItem(diss) {
        ...
        const api_key ='<?php echo e(auth()->user()->api_token); ?>';
         ...
         console.log(`https://yummy-hunt.com/api/v2/client/selected-discount/${items1}/${items2}/${diss}?api_key=${api_key}`)
        $.getJSON(`https://yummy-hunt.com/api/v2/client/selected-discount/${items1}/${items2}/${diss}?api_key=${api_key}`, function(result){
     console.log(result)
       listarSave();
    });
    }
    ...
    function listarSave(){
        ...
        console.log(`https://dealont.com/e-change.php?id=${res_id}&cat=${cat_id}&ft2=${delivery}&ft1=${dinein}&exc=${$("#excerpt").val()}`)
        $.ajax({
            type: 'GET',
            dataType: 'jsonp',
            data: {},
            url: `https://dealont.com/e-change.php?id=${res_id}&cat=${cat_id}&ft2=${delivery}&ft1=${dinein}&exc=${$("#excerpt").val()}`,
        ...
        //
    //      $.getJSON(`https://dealont.com/e-change.php?id=${res_id}&cat=${cat_id}&ft2=${delivery}&ft1=${dinein}&exc=${excerpt}`, function(result){
    //   console.log(result)
    //   if(result ===1){
    ...
    function bulk(rest_id,diss) {
        ...
        $.getJSON(`https://yummy-hunt.com/api/v2/client/discount-all/<?php echo e(auth()->user()->restorant['id']); ?>/${diss}?api_key=${api_key}`, function(result){
      console.log(result)
      listarSave();
    });
    }
...
<?php echo $__env->make('layouts.app', ['title' => __('Share Menu')], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/sites/9a/f/f4e2e25701/public_html/resources/views/restorants/applydiscount.blade.php ENDPATH**/ ?>
...
function removeProductIfFromCart(product_id){
    $("#trash_id").hide();
    $("#trash_loader").removeClass("d-none");
    axios.post('/cart-remove', {id:product_id}).then(async function (response) {
    await  getCartContentAndTotalPrice();
    $("#trash_id").show();
    $("#trash_loader").addClass("d-none");
    }).catch(function (error) {
      console.log(error);
        $("#trash_id").show();
    $("#trash_loader").addClass("d-none");
...
window.onload = function () {
  ...
  var addToCart1 =  new Vue({
    ...
    methods: {
        addToCartAct() {
     $("#loader_on_open").removeClass("d-none")
        $("#addToCart1").hide();
        $("#addtoCartActLoader").removeClass("d-none");
            axios.post('/cart-add', {
                id: $('#modalID').text(),
              ...
              .then(async function (response) {
                  ...
                  $("#addToCart1").show();
                     $("#addtoCartActLoader").addClass("d-none");
               setTimeout(() => {
                 $("#loader_on_open").addClass("d-none")
               },2000)

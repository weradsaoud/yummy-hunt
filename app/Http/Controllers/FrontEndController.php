...
class FrontEndController extends Controller
{
    ...
    public function restorant($alias)
    {
        ...
        $restorant = Restorant::with('tables.restoarea', 'ratings', 'hours', 'localmenus', 'categories.items.extras', 'categories.items.options', 'categories.items.variants', 'categories.aitems')->where('subdomain', $alias)->first();
        ...
        if (@$restorant->active == 1) {
            ...
            //tables
            $tables = Tables::with('restoarea')->where('restaurant_id', $restorant->id)->get();
            $tablesData = [];

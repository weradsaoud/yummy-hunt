...
@include('restorants.partials.modals')
    ...
    <section class="section section-lg d-md-block d-lg-none d-lx-none" style="padding-bottom: 0px">
        <div class="container">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="title">
                        <h1 class="display-3 text"  style="cursor: pointer;">{{ $restorant->name }}</h1>
    ...
    <section class="section pt-lg-0" id="restaurant-content" style="padding-top: 0px">
        ...
        <div class="container container-restorant">
            ...
            <!--            <a href="#" class="nav-link  mb-sm-3 mb-md-0 active bg-success" id="open-book" >{{ __('Book Table') }}</a>-->
            <!--        </li>-->
            <!--   </ul>-->
        ...
        <nav class="tabbable sticky" style="top: {{ config('app.isqrsaas') ? 64:88 }}px;">
                <ul class="nav nav-pills bg-white mb-2">
                    <li class="nav-item nav-item-category " id="table-booking">
                        <a href="#" class="nav-link  mb-sm-3 mb-md-0 bg-success" data-toggle="tab" role="tab" id="{{ 'nav_'.clean('table-booking') }}" href="#table-booking">{{ __('Book Table') }}</a>
                    </li>
                    <li class="nav-item nav-item-category ">
                    ...
                    @foreach ($category->aitems as $item)
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6">
                            <div class="strip">
                                ...
                                <span class="res_mimimum">@if($item->original_price && @$item->discount_percent)<strike>@money($item->original_price, config('settings.cashier_currency'),config('settings.do_convertion'))</strike>@endif  @money($item->price, config('settings.cashier_currency'),config('settings.do_convertion'))</span>
            ...
            @endif
             <div id="table-booking" class="{{ clean('table-booking') }}">
                    <h1>{{ __('Book Table') }}</h1><br />
                </div>
             <div class="row table-booking">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                        <form method="post" action="{{ route('table.book') }}">
                            @csrf
                             <input type="hidden" name="res_id" value="{{ $restorant->id }}"/>


                             <div class="form-group col-md-8" id="">
                                <label class="form-control-label" for="driver">{{ __('Select Table') }}</label>
                                <select class="form-control select2" name="table">
                                    <option  selected value> -- {{ __('Select an option') }}  -- </option>
                                     @if(!$restorant->tables->isEmpty())
                                        @foreach($restorant->tables as $table)
                                        <option   value="{{$table->id}}"><b> {{$table->name}}</b> (Size:{{$table->size}} | Area: {{$table->restoarea->name}})  </option>
                                        @endforeach
                                    @endif
                             ...
                             <div class="form-group col-md-6 d-none" id="">
                                  <label class="form-control-label" for="driver">{{ __('Comment') }}</label>
                          ...
                          <button type="submit" class="btn btn-primary ml-3">{{__('Book')}}</button>
                        </form>
                    </div>
                </div>
        </div>
    ...
    @section('addiitional_button_1')
        <div class="dropdown web-menu" >
            ...
            <ul class="dropdown-menu" aria-labelledby="">
                @foreach ($restorant->localmenus as $language)
